...
> **ファイルワークベンチ + インタラクション領域の階層化** —— ファイルは読み取り専用プレビューから編集可能なワークベンチへ、エージェントの思考は走査しやすい 3 層ストリームへ。

### v0.31.0 · ファイルワークベンチ

- **本物の編集（CM6 カーネル）** —— フローティングウィンドウがそのままエディタに：直接編集、アトミック保存、競合時は 3 択、未保存のまま閉じる際は保護が入ります。
- **ファイルツリー自動リフレッシュ** —— chokidar がワークスペースを監視：エージェントが 10 ファイルを書けば 10 か所すべて自動更新。カード / ツリー / タブのバッジは単一の状態ソースから供給されます。
- **Goto Anything** —— `⌘P` で `ファイル@シンボル:行` へ直行。エディタ内検索、選択アクション、ツリーからの新規作成 / ドラッグ＆ドロップも一体化。
- **成果物カード** —— エージェントが書いたファイルは会話フローにカードで現れ、クリックでフローティングウィンドウがそのファイル位置で開きます。

### v0.31.0 · インタラクション領域の再構築

- **本物の思考がついに見える** —— ネイティブ `reasoning_content` / `thinking_delta` を UI が受け取り永続化。プロトコルマーカーはデルタ粒度で剥離、リークゼロ。
- **Turn → Step → Block の 3 層** —— 意図 / 思考 / ツール / 結論が形 + アイコン + 色で区別されます。6 状態マシン + 並列ツールグループ。ネイティブ tooltip はホバーカードに置き換え。
- **キーバインド中央化** —— 全ショートカットを `KeybindingRegistry` へ収録（13 分支を動作変更ゼロで移行）、ヘルプセンターで確認可能。

### v0.31.0 · ユーザー裁定の仕上げ（C1–C3）

- **C1** —— エディタは「編集 / 読み取り専用レンダー」の 2 モードに集約。markdown 分割ビューは同期スクロール。折りたたみヘッダーは固定の「思考中」ラベル。
- **C2** —— タスクタイトルを LLM が内容から生成（16 文字以内、手動改名でロック）—— 「無題タスク」だらけはもうありません。
- **C3** —— 修正：最小化したプレビューウィンドウを復元しても空にならない（ウィンドウ全体のスナップショット + スワップ保護 + 専用の破棄アクション）。

> 検証済み：`typecheck` exit 0 · **84 スイート / 875 ケース、fail 0** · i18n 4 言語キーセット一致 · パッケージ済み成果物の起動スモーク合格。
