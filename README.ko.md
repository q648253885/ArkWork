...
> **파일 워크벤치 + 인터랙션 영역 계층화** —— 파일은 읽기 전용 미리보기에서 편집 가능한 워크벤치로, 에이전트의 사고 과정은 훑기 쉬운 3계층 스트림으로.

### v0.31.0 · 파일 워크벤치

- **진짜 편집(CM6 커널)** —— 플로팅 창이 곧 에디터: 직접 수정, 원자적 저장, 충돌 시 3택, 저장하지 않고 닫으면 보호가 작동합니다.
- **파일 트리 자동 새로고침** —— chokidar가 워크스페이스를 감시: 에이전트가 파일 10개를 쓰면 10곳 모두 자동 갱신. 카드 / 트리 / 탭 배지는 단일 상태 소스에서 공급됩니다.
- **Goto Anything** —— `⌘P`로 `파일@심볼:행` 바로 이동. 에디터 내 찾기, 선택 동작 시스템, 트리에서 새로 만들기 / 드래그 앤 드롭까지 통합.
- **산출물 카드** —— 에이전트가 쓴 파일은 대화 흐름에 카드로 나타나며, 클릭하면 플로팅 창이 해당 파일 위치로 열립니다.

### v0.31.0 · 인터랙션 영역 재구축

- **진짜 사고가 드디어 보임** —— 네이티브 `reasoning_content` / `thinking_delta`를 UI가 수신하고 영속화. 프로토콜 마커는 delta 단위로 제거, 누출 0.
- **Turn → Step → Block 3계층** —— 의도 / 사고 / 도구 / 결과가 형태 + 아이콘 + 색으로 구분됩니다. 6상태 머신 + 병렬 도구 그룹. 네이티브 tooltip은 호버 카드로 대체.
- **키 바인딩 중앙화** —— 모든 단축키를 `KeybindingRegistry`로 수록(13개 분支을 동작 변경 없이 이전), 도움말 센터에서 확인 가능.

### v0.31.0 · 사용자 결정 마무리(C1–C3)

- **C1** —— 에디터는 편집 / 읽기 전용 렌더 2 모드로 수렴. markdown 분할 보기는 동기 스크롤. 접힌 헤더는 고정된 "생각 중" 레이블.
- **C2** —— 작업 제목을 LLM이 내용에서 생성(16자 이내, 수동 이름 변경 시 잠금) — "제목 없는 작업" 벽은 끝.
- **C3** —— 수정: 최소화한 미리보기 창을 복원해도 비어 있지 않음(전체 창 스냅샷 + 스왑 보호 + 전용 삭제 액션).

> 검증: `typecheck` exit 0 · **84개 스위트 / 875개 케이스, fail 0** · i18n 4개 언어 키셋 일치 · 패키징 산출물 기동 스모크 통과.
