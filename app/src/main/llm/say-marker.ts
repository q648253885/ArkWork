...
/* ============================================================
 * v0.31.0 B1：把标记字面量与阈值提升为「具名导出」。
 *
 * 为什么要导出（而不是继续留在模块内）：
 *  - `main/llm/stream-strip.ts`（流式期 delta 级剥离）必须与本文（落定期整串解析）
 *    使用**同一阈值**，否则两条路径会在边界输入上给出不一致的渲染分流；
 *  - 若各自硬编码，改一处忘另一处 → 静默分叉（正本 §6.1 硬约束、TC-STREAM-006 把守）。
 *  - 语义零变化：既有 `extractSayMarker` 行为逐字节不变，10 条既有用例不受影响。
 * ============================================================ */

/** SAY 块正文长度上限（超出即截断加省略号）。落定期与流式期共用同一常量。 */
export const MAX_SAY_CHARS = 600

/**
 * SAY 块开标记（无正则元字符，供流式剥离器做「字面前缀匹配」）。
 * 注意：与 `SAY_OPEN` 正则可不同——本常量**不含尾部空白容忍**，
 * 因为流式期逐字符到达时无法判定「空白是否已结束」。
 */
export const SAY_OPEN_TOKEN = '<<<SAY>>>'

/** SAY 块闭合标记（同上，字面量形态） */
export const SAY_CLOSE_TOKEN = '<<<END>>>'
