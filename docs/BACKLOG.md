...
## 四·A、v0.34.2 遗留（L-34-01…04）

| # | 项 | 影响 | 去向 | 状态 |
|---|---|---|---|---|
| L-34-01 | 面板数据源 `fallbackUrls`（主源不可达时自动切备源） | 本次实测 `push2` 不可达只能靠**换主机**解决；插件声明式无备源能力 → 下次源站抖动仍会「打不开」 | v0.35 | open |
| L-34-02 | 面板 http 源 `charset=GBK` 解码 | 腾讯/新浪行情接口是 GBK，宿主 `res.text()` 按 UTF-8 解 → 名称乱码；本版靠「只用 UTF-8 JSON 源」规避，未真正支持 | v0.35 | open |
| L-34-03 | 插件面板「侧边栏 / 浮窗」surface 声明 | 股票插件 3 个面板全部成为竖排栏 Tab，而作者本意是「1 侧边栏 + 2 浮窗」；声明式 surface 待设计（当前靠 `moreTabs` 折叠缓解） | v0.35 | open |
| L-34-04 | 浮窗（MinimizedCapsule）插件插槽 | 插件可控制范围内唯一未打通的宿主（`MEMORY.md` 插件边界条目） | v0.35 | open |
| L-34-05 | v0.34.1 文档目录缺失（`docs/versions/v0.34.1/`） | 该版已 tag 发布但无设计文档；代码注释里引用的 `docs/versions/v0.34.1/04-system-design.md` §5 是断链 | 待定（可回填或在下版承认） | open |
...
| v1.1 | 2026-09-19 | 登记 v0.34.2 遗留 L-34-01…05（备源机制 / GBK 解码 / 面板 surface / 浮窗插槽 / v0.34.1 文档缺目录） |
