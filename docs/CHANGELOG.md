...
| **v0.34.2** | **2026-09-19** | **补丁版：D56 插件面板打不开（取数栈改 Chromium `net.fetch` 遵循系统代理 + 数据源主机 push2 → push2delay，实测 push2 为 ERR_EMPTY_RESPONSE）/ D56-c 竖排栏高度自适应折叠（可见名称 = min(3, 高度可容纳)，口径纠正取代 D54）/ D57 随包示例「未改动即升级」（修正送得到存量机器）** | **已实现（1374 条全绿 + 双 tsc 零错 + build 通过；mac `.app` 已重出）。⚠️ 提交/打 tag 待用户确认** |
